 ...
 * @param {Document} doc
 * @param {{ size?: number }} [state]
 * @returns {SVGElement} a closed book, for the read later button
 */
export function bookIcon(doc, { size = 14 } = {}) {
  return build(doc, OUTLINE, 'book', size);
}

/**
 * @param {Document} doc
 * @param {{ size?: number }} [state]
 * @returns {SVGElement} a clock, for the button that saves the page for later
 */
export function clockIcon(doc, { size = 14 } = {}) {
  return build(doc, OUTLINE, 'clock', size);
}

/**
 * @param {Document} doc
 * @param {{ size?: number }} [state]
 * @returns {SVGElement} a cog, for the settings button
 */
export function gearIcon(doc, { size = 15 } = {}) {
  return build(doc, OUTLINE, 'gear', size);
}

/**
 * @param {Document} doc
 * @param {{ filled?: boolean, size?: number }} [state]
 * @returns {SVGElement} a tick in a circle, filled once the bookmark is read
 */
export function checkIcon(doc, { filled = false, size = 14 } = {}) {
  return build(doc, filled ? FILLED : OUTLINE, 'check', size);
}
